...
public class ToFrontCommand implements Command{
			...
			while(it1.hasNext()){
				...
				if(shape1.toString().equals(shape.toString())){
					...
					if(j!=model.getShapes().size()-1) {
						Collections.rotate(model.getShapes().subList(j, j+2), 1);
					} else {
						executed = false;
		...
		if(!model.getShapes().isEmpty()) {
			...
			while(it1.hasNext()){
				...
				if(shape1.toString().equals(shape.toString())){
					int j = model.getShapes().indexOf(shape1);
					if(j!=0) {
						Collections.rotate(model.getShapes().subList(j-1, j+1), 1);
					} else {
						executed = false;
	...
	public String appendLog() {
		String text = "TO FRONT COMMAND shape: " + shape.getClass().getSimpleName() + " : " + shape.toString() + ":" +shape.getColor().getRGB();
